...
class Humanoid(BaseTask):
    ...
    def reset(self, env_ids=None, randomize=True):
        ...
        self._reset_envs(env_ids, randomize)
        return
    ...
    def _reset_envs(self, env_ids, randomize=True):
        if (len(env_ids) > 0):
            if len(env_ids)==1:
                env_ids = env_ids.view(-1)
            self._reset_actors(env_ids, randomize=randomize)
    ...
    def _build_env(self, env_id, env_ptr, humanoid_asset):
        ...
        # foot_geom_name =[11, 14] # right left foot
        # props = self.gym.get_actor_rigid_shape_properties(env_ptr, humanoid_handle)
        # for p_idx in range(len(props)):
        #     print(props[p_idx].restitution, props[p_idx].friction, props[p_idx].rolling_friction, props[p_idx].torsion_friction)
        #     if p_idx in foot_geom_name:
        #         props[p_idx].restitution = 0.01
        #         props[p_idx].friction = 1.8
        #         props[p_idx].rolling_friction = 2.0
        #         props[p_idx].torsion_friction = 2.0
    ...
    def _compute_reward(self, actions):
        self.rew_buf[:] = compute_humanoid_reward(self.obs_buf) #[check] should not come here!!!
        return
    ...
    def post_physics_step(self):
        ...
        self.extras['metrics'] = self._evaluate_metrics()
    ...
    def _init_camera(self):
        ...
        cam_pos = gymapi.Vec3(self._cam_prev_char_pos[0]-1.0, 
                              ...
                              4.0)
        cam_target = gymapi.Vec3(self._cam_prev_char_pos[0]+4.0,
                                 self._cam_prev_char_pos[1]+4.0,
                                 1.0)
        self.gym.viewer_camera_look_at(self.viewer, None, cam_pos, cam_target)
        if len(self.camera_handles) > 0:
            self.gym.set_camera_location(self.camera_handles[0], self.envs[0], cam_pos, cam_target)
