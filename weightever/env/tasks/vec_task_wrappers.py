...
import numpy as np
import torch
from weightever.env.tasks.vec_task import VecTaskCPU, VecTaskGPU, VecTaskPython
...
class VecTaskPythonWrapper(VecTaskPython):
    ...
    def reset(self, env_ids=None, randomize=True):
        self.task.reset(env_ids, randomize=randomize)
...
class VecNCPythonWrapper(VecTaskPython):
    def __init__(self, task, rl_device, clip_observations=5.0, clip_actions=1.0):
        super().__init__(task, rl_device, clip_observations, clip_actions)
